...
from langchain_core.models.safe_retriever import AuthContext, SemanticContext
...
class VectorStore(ABC):
    ...
    def as_safe_retriever(self, **kwargs: Any) -> SafeRetriever:
        """Return SafeRetriever initialized from this VectorStore.
        ...
        Returns:
            SafeRetriever: Retriever class for VectorStore.
...
class SafeRetriever(VectorStoreRetriever):
    """Safe Retriever class for VectorStore."""

    auth_context: Optional[AuthContext] = Field(None, exclude=True)
    """Authorized context for the retriever."""

    semantic_context: Optional[SemanticContext] = Field(None, exclude=True)
    """Semantic context for the retriever."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Add safe_retrival flag to search_kwargs
        self.search_kwargs["safe_retrieval"] = True

    def _get_relevant_documents(
        self, query: str, *, run_manager: CallbackManagerForRetrieverRun
    ) -> List[Document]:
        self.set_safe_retrieval_contexts()
    ...
    def set_safe_retrieval_contexts(self):
        """
        Set the auth_context and semantic_context to the search_kwargs
        """
        self.search_kwargs["auth_context"] = self.auth_context
        self.search_kwargs["semantic_context"] = self.semantic_context
